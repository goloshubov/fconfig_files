...
alias k_no_gpu="kubectl get nodes -o=go-template='{{range .items}}{{.metadata.name}}{{printf \"\t\"}}{{.status.capacity.cpu}}{{printf \"\t\"}}{{.status.capacity.memory}}{{printf \"\t\"}}{{index .status.capacity \"nvidia.com/gpu\"}}{{printf \"\n\"}}{{end}}' | sort -n -k 4 -k 2"
